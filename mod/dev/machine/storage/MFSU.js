...
MachineRegistry.registerPrototype(BlockID.storageMFSU, {
	...
	tick: function(){
		...
		this.data.energy += ChargeItemRegistry.getEnergyFrom(this.container.getSlot("slot2"), Math.min(this.data.TRANSFER, energyStorage - this.data.energy), 2);
		this.data.energy += ChargeItemRegistry.addEnergyTo(this.container.getSlot("slot1"), Math.min(this.data.TRANSFER, this.data.energy), 2) - Math.min(this.data.TRANSFER, this.data.energy);
	},
